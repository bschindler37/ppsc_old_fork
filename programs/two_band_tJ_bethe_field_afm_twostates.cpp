...
#include "ppsc/hilbert_spaces/two_band_fermi_tJ_two.hpp"
#include "ppsc/hamiltonians/two_band_tJ_pos.hpp"
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // boson bath
      ...
      green_equilibrium(D0,dos1,beta,h,30,30,mu_bath);
      cntr::function<double> gfunc(nt);
      for(int tstp=-1;tstp<=nt;tstp++){
        cdmatrix tmp(1,1);
        tmp(0,0)=g[tstp+1];
        gfunc.set_value(tstp,tmp);
        std::cout << "tu smo 4 " << g[tstp+1] << std::endl;
      }
    ...
    // -------------------------------------------------------------------
    // -- Setup pp calculator
    typedef ppsc::hilbert_spaces::two_band_fermi_tJ_two hilbert_space_type;
    typedef ppsc::hamiltonians::two_band_tJ_pos<hilbert_space_type> hamiltonian_type;
