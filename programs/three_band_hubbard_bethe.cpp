...
#include "./ppsc/hamiltonians/three_band_hubbard.hpp"
...
public:
  ...
  void update(int tstp, single_particle_greensfunction_type & Gloc,
	      cntr::function<double> & tfunc1, cntr::function<double> & tfunc2, cntr::function<double> & tfunc3,
	      cntr::herm_matrix<double> & D0, // boson bath
	      int af_bethe_flag=0, int bath_flag=0) {

    // -- Update Delta.o1up
    if(af_bethe_flag == 1) _component_update(tstp, this->o1up, this->o1up_cc, Gloc.o1do, tfunc1, D0, bath_flag);
    else                   _component_update(tstp, this->o1up, this->o1up_cc, Gloc.o1up, tfunc1, D0, bath_flag);

    // -- Update Delta.o1do
    if(af_bethe_flag == 1) _component_update(tstp, this->o1do, this->o1do_cc, Gloc.o1up, tfunc1, D0, bath_flag);
    else                   _component_update(tstp, this->o1do, this->o1do_cc, Gloc.o1do, tfunc1, D0, bath_flag);

    // -- Update Delta.o2up
    if(af_bethe_flag == 1) _component_update(tstp, this->o2up, this->o2up_cc, Gloc.o2do, tfunc2, D0, bath_flag);
    else                   _component_update(tstp, this->o2up, this->o2up_cc, Gloc.o2up, tfunc2, D0, bath_flag);

    // -- Update Delta.o2do
    if(af_bethe_flag == 1) _component_update(tstp, this->o2do, this->o2do_cc, Gloc.o2up, tfunc2, D0, bath_flag);
    else                   _component_update(tstp, this->o2do, this->o2do_cc, Gloc.o2do, tfunc2, D0, bath_flag);

    // -- Update Delta.o3up
    if(af_bethe_flag == 1) _component_update(tstp, this->o3up, this->o3up_cc, Gloc.o3do, tfunc3, D0, bath_flag);
    else                   _component_update(tstp, this->o3up, this->o3up_cc, Gloc.o3up, tfunc3, D0, bath_flag);

    // -- Update Delta.o3do
    if(af_bethe_flag == 1) _component_update(tstp, this->o3do, this->o3do_cc, Gloc.o3up, tfunc3, D0, bath_flag);
    else                   _component_update(tstp, this->o3do, this->o3do_cc, Gloc.o3do, tfunc3, D0, bath_flag);
  ...
  void _component_update(int tstp,
    ...
    Delta_comp.left_multiply( tstp, tfunc);
    Delta_comp.right_multiply(tstp, tfunc);
...
int main(int argc, char *argv[]) {
  ...
  std::vector<double> t_vec, t_vec1, t_vec2, t_vec3;
  ...
  try {
    ...
    typedef ppsc::hamiltonians::three_band_hubbard<
      hilbert_space_type,
    ...
    cntr::function<double> tfunc1(nt);
    for(int tstp=-1; tstp <= nt; tstp++) {
      tfunc1[tstp] = t_vec1[tstp+1];
    }
    cntr::function<double> tfunc2(nt);
    for(int tstp=-1; tstp <= nt; tstp++) {
      tfunc2[tstp] = t_vec2[tstp+1];
    }
    cntr::function<double> tfunc3(nt);
    for(int tstp=-1; tstp <= nt; tstp++) {
      tfunc3[tstp] = t_vec3[tstp+1];
    }
	...
	Delta.update(-1, Gloc, tfunc1, tfunc2, tfunc3,
    ...
    if (nt > 0 && matsubara_converged == true) {
	  ...
	  Delta.update(n, Gloc, tfunc1, tfunc2, tfunc3,
		       D0, af_bethe_flag, bath_flag);
        }
	...
	Delta.update(tstp, Gloc, tfunc1, tfunc2, tfunc3,
		     D0, af_bethe_flag, bath_flag);
