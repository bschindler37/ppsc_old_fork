...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    double omega; // -- boson bath frequency
    std::vector<double> g; // -- boson bath coupling
    
    find_param_tvector(argv[1], "__g=", g, nt);
    find_param(argv[1], "__omega=", omega);

    // -- construct cntr-funct for coupling
	...
	Delta_up.set_timestep(-1, tmp); // Bethe lattice contrib
	
	if(bath_flag) {
	  Bubble2(-1, tmp, Gloc_up, D0);
	  Delta_up.incr_timestep(-1, tmp);
	}
	...
	for (int n = 0; n <= kt; n++) {
	  ...
	  if(bath_flag) {
	    Bubble2(n, tmp, Gloc_up, D0);
	    Delta_up.incr_timestep(n, tmp);
	  }
	...
	if(bath_flag) {
	  Bubble2(tstp, tmp, Gloc_up, D0);
	  Delta_up.incr_timestep(tstp, tmp);
	}
