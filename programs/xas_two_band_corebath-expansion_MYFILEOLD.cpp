...
void initialize_hamiltonian(HAMILTONIAN & ham_, double Hubbard_U, double e_v, double e_c, double g_num, int nt, double h, double beta, double omega_in, double sigma_pulse, double mean_pulse ){
	...
	for (int tstp = -1; tstp <= nt; tstp++) {
		if (tstp >= 0) {
			...
			ham_.probe_pulse_env[tstp+1] = std::exp(- (time_diff_aux * time_diff_aux) / (2*sigma_pulse*sigma_pulse) );
...
int main(int argc, char *argv[]) {
	...
	ppsc::gf_verts_type gf_verts_CBEXP, gf_verts_CBCON;	// is the same for core-bath-hybridization-expansion & core-bath-convolution at the end
	if (SCREEN_HYB) {
	...
	gf_verts_CBEXP = get_gf_verts(hilbert_space); gf_verts_CBCON = get_gf_verts(hilbert_space);
	imp_CBEXP.update_diagrams(pp_ints_CBEXP, gf_verts_CBEXP); imp_CBCON.update_diagrams(pp_ints_CBCON, gf_verts_CBCON);		// -- tell solver which diagrams to evaluate when performing 'get_spgf' or when doing 'pp_step'
	...
	// -- Update temporary GFs according to atomic/initialized solution above (set_t0_from)
	for( auto idx : ppsc::range(0, gf_tstps_CBEXP.size()) ) {
	 	G_loc_CBEXP_temp.set_matrixelement(-1, gf_verts_CBEXP[idx].idx1, gf_verts_CBEXP[idx].idx2, gf_tstps_CBEXP[idx]);
		G_loc_CBCON_temp.set_matrixelement(-1, gf_verts_CBCON[idx].idx1, gf_verts_CBCON[idx].idx2, gf_tstps_CBCON[idx]);
	}
	...
	for ( iter = 1 ; iter <= itermax; iter++) {
		...
		// -- Update G_loc
		for( auto idx : ppsc::range(0, gf_tstps_CBEXP.size()) ) {
		 	G_loc_CBEXP.set_matrixelement(-1, gf_verts_CBEXP[idx].idx1, gf_verts_CBEXP[idx].idx2, gf_tstps_CBEXP[idx]);
			G_loc_CBCON.set_matrixelement(-1, gf_verts_CBCON[idx].idx1, gf_verts_CBCON[idx].idx2, gf_tstps_CBCON[idx]);
		}
	...
	if (nt > 0){
		...
		for ( iter = 1 ; iter <= itermax; iter++) {
			...
			for (int tstp = 0; tstp <= kt; tstp++) {
				...
				// -- Update G_loc
				for( auto idx : ppsc::range(0, gf_tstps_CBEXP.size()) ) {
				 	G_loc_CBEXP.set_matrixelement(tstp, gf_verts_CBEXP[idx].idx1, gf_verts_CBEXP[idx].idx2, gf_tstps_CBEXP[idx]);
					G_loc_CBCON.set_matrixelement(tstp, gf_verts_CBCON[idx].idx1, gf_verts_CBCON[idx].idx2, gf_tstps_CBCON[idx]);
				}
	...
	cntr::vie2_start(G_loc_CBCON_new, F, F_cc, G_loc_CBCON, beta, h, kt);
		...
		G_loc_CBCON.set_timestep(tstp, G_loc_CBCON_new);
      	...
      	// -- Calculate xas_integrand = II * exp(-II * omega_in * t) * s(t) * < H_in^mat (t) > / g 
      	// -- with H_in^mat (t) = P_in_exp(t) , a quantity depending on omega_in (!!!)
      	...
      	for( int tstp = 0; tstp <= nt; tstp++ ) {
      	      	...
      	      	P_in_CBCON[tstp] = rho(0,2) + rho(1,3);
      		...
      		xas_integrand_CBCON[tstp] = h * 2 * II * std::exp(-II * omega_in * double(tstp) * h) * ( rho(0,2) + rho(1,3) ) * (imp_CBCON.hamiltonian.probe_pulse_env[tstp+1]) / g_num ;
	...
	if (SCREEN_HYB) {
		output_dir << "./test_output_xas_new/" << "Omega" << Omega_0 << "_gammatilde" << gammatilde << "_Gamma" << Gamma << "_ev" << e_v << "_ec" << e_c << "_U" << Hubbard_U << "_ntau" << ntau << "_nt" << nt << "_beta" << beta << "_h" << h << "_pulse-mean" << mean_pulse << "_pulse-sigma" << sigma_pulse << "_corebandwidth" << core_bandwidth << "_gnum" << g_num; 
	}
	else {
		output_dir << "./test_output_xas_new/" << "NOSCREEN" << "_Gamma" << Gamma << "_ev" << e_v << "_ec" << e_c << "_U" << Hubbard_U << "_ntau" << ntau << "_nt" << nt << "_beta" << beta << "_h" << h << "_pulse-mean" << mean_pulse << "_pulse-sigma" << sigma_pulse << "_corebandwidth" << core_bandwidth << "_gnum" << g_num ;
	}
	
	output_file_xas << "/" << "I_XAS_4.txt" ;
	output_file_time_CBEXP << "/" << "imag-P_in_exp_CBEXP_4.txt" ;
	output_file_time_CBCON << "/" << "imag-P_in_exp_CBCON_4.txt" ;
	output_file_time_CBCON_no_corebath << "/" << "imag-P_in_exp_CBCON_no_corebath_4.txt" ;
	...
	if ( ! file_exists(output_filename_xas) && ! file_exists(output_filename_CBEXP) && ! file_exists(output_filename_CBCON) && ! file_exists(output_filename_CBCON_no_corebath) ) {
		...
		P_in_CBEXP_file << "# omega_in" << "\t" << "<P_in>/g (t=0)" << "\t" << "<P_in>/g (h)" << "\t" << "<P_in>/g (2*h)" << "\t" << "..." ;
		//
		...
		P_in_CBCON_file << "# omega_in" << "\t" << "<P_in>/g (t=0)" << "\t" << "<P_in>/g (h)" << "\t" << "<P_in>/g (2*h)" << "\t" << "..." ;
		//
		...
		P_in_CBCON_no_corebath_file << "# omega_in" << "\t" << "<P_in>/g (t=0)" << "\t" << "<P_in>/g (h)" << "\t" << "<P_in>/g (2*h)" << "\t" << "..." ;
		//
	...
	if ( I_XAS_file.is_open() && P_in_CBEXP_file.is_open() && P_in_CBCON_file.is_open() && P_in_CBCON_no_corebath_file.is_open() ) {
		...
		for ( int tstp = 0; tstp<= nt; tstp++ ) {
			P_in_CBEXP_file << "\t" << imp_CBEXP.hamiltonian.P_in_exp[tstp+1] / g_num ;
			P_in_CBCON_file << "\t" << P_in_CBCON[tstp] / g_num ;
			P_in_CBCON_no_corebath_file << "\t" << imp_CBCON.hamiltonian.P_in_exp[tstp+1] / g_num ;
		}
	...
	// ---------------------------------------------------------------------
	// PRINT OUT / SAVING _REAL-TIME_ RESULTS FOR < P_in >_{g_num} / g_num 
