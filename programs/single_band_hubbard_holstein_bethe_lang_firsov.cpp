...
#include "./ppsc/solver_lang_firsov.hpp"
...
#include "./ppsc/hamiltonians/single_band_hubbard_lang_firsov.hpp"
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    typedef ppsc::hamiltonians::single_band_hubbard_lang_firsov<hilbert_space_type> hamiltonian_type;
    typedef ppsc::solver_lang_firsov<hamiltonian_type> solver_type;
    ...
    { // -- Setup local Hamiltonian
      ...
      find_param(argv[1], "__g=", imp.hamiltonian.g);
      find_param(argv[1], "__omega=", imp.hamiltonian.omega);
    ...
    */
    ...
    /* 
    // -- dress the boson propagator with the coupling constant g * D * g
    for(int tstp=-1; tstp <= nt; tstp++) {
    ...
    */
	...
	imp.add_phonon_kernel(-1, Gloc_up);
	...
	imp.add_phonon_kernel(-1, Delta_up);
    ...
    if (nt > 0 && matsubara_converged == true) {
	  ...
	  imp.add_phonon_kernel(n, Gloc_up);
	  
          cntr::herm_matrix_timestep<double> tmp;
          Gloc_up.get_timestep(n, tmp);
          Delta_up.set_timestep(n, tmp);

	  imp.add_phonon_kernel(n, Delta_up);
	...
	chi.set_timestep(tstp, gf_tstps[1]); 

	imp.add_phonon_kernel(tstp, Gloc_up);

        cntr::herm_matrix_timestep<double> tmp;
        Gloc_up.get_timestep(tstp, tmp);
	Delta_up.set_timestep(tstp, tmp);

	imp.add_phonon_kernel(tstp, Delta_up);
