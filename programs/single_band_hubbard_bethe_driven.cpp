...
void DMFT_selfconsisteny(int tstp, cntr::herm_matrix<double> &G,
                         cntr::herm_matrix<double> &D,
                         cntr::function<double> &J) {
  // compute Delta from G:
  // here we use a simple Bethe selfconsistency with external field.
  // replace this by anything more complicated (e.g. if you have a real lattice)
  // eith real hopping the bethe SC is
  // Delta(t,t') = J**2 G(t,t')
  // here we use a complex hopping J(t) (with Peierls phase)
  // 1/2 of the neigboring sites are in direction of the field, with hopping
  // J(t) 1/2 of the neigboring sites are against the field, with hopping J(t)^*
  // -->  Delta(t,t') = 0.5 [ J(t)G(t,t')J(t')* +  J(t)*G(t,t')J(t') ]
  cntr::herm_matrix_timestep<double> tmp(tstp, G.ntau());
  tmp.set_timestep(tstp, G);
  tmp.left_multiply(tstp, J, 0.5);
  tmp.right_multiply_hermconj(tstp, J, 1.0);
  D.set_timestep(tstp, tmp);
  tmp.set_timestep(tstp, G);
  tmp.left_multiply_hermconj(tstp, J, 0.5);
  tmp.right_multiply(tstp, J, 1.0);
  D.incr_timestep(tstp, tmp);
}

/* ------------------------------------------------------------------------------
For the model above current and kinetic energy are then obtained by looking
at the equations of motion (only sketch ... )

I*Dt G_{ii}(t,t') - \sum_{a} J0 e^{I*A(t)*r_{ia}} G_{ai}(t,t') + [...] =
del(t,t')

where [...] are local and interaction contributions --> for  C=Delta*G
C(t,t') = \sum_{a} J0 e^{I*A(t)*r_{ia}} G_{ai}(t,t')
==>  C_les(t,t) = I*sum_{a} J e^{I*A(t)*r_{ia}} <c_i^dag c_a>
<Ekin(t)> = 1/2 * (-I) * [(DL+DR)*G]_les(t,t)

For the current operator:
J = -dH/dA = -I*\sum_{<ij>} J0 * r_{ij} * e^{I*A(t)*r_{ij}} c_i^dag c_j
such that  d/dt <H(t)> = < dH/dt > = < dH/dA * dA/dt > = <J(t)>*Efield(t)
Similar derivations as for Ekin give <J> = -(I/2) * (-I) * [(DL-DR)*G]^<(t,t)

In summary: We can compute R(t) = -I*[DL*G]_les(t,t) and get
Ekin(t) = re R(t), J(t)    = im R(t)
------------------------------------------------------------------------------
*/
void get_Ekin(vector<double> &Ekin, vector<double> &current,
              cntr::herm_matrix<double> &G, cntr::function<double> &J, int kt,
              double beta, double h) {
  int nt = G.nt();
  cntr::herm_matrix<double> DL = G;
  Ekin.resize(nt + 2);
  current.resize(nt + 2);
  for (int tstp = -1; tstp <= nt; tstp++) {
    DL.left_multiply(tstp, J);
    DL.right_multiply_hermconj(tstp, J, 1.0);
  }
  cdmatrix rho(1, 1);
  for (int tstp = -1; tstp <= nt; tstp++) {
    cntr::convolution_density_matrix(tstp, rho, DL, G, beta, h, kt);
    Ekin[tstp + 1] = 2 * rho(0, 0).real(); // 2 for spin
    current[tstp + 1] = 2 * rho(0, 0).imag();
  }
}
  ...
  try {
    ...
    // MATSUBARA PART (EQUILIBRIUM INITIAL STATE)
    {
      ...
      for (iter = 1; iter <= itermax; iter++) {
        ...
        // -- Update Hybridization: The DMFT self-consistency
        // to compute Delta from G at timestep tstp=-1
        if (!atomic_limit) {
          DMFT_selfconsisteny(-1, Gloc_up, Delta_up, J);
        }
    ...
    if (nt > 0 && matsubara_converged == true) {
      ...
      for (iter = 1; iter <= itermax; iter++) {
        ...
        for (int n = 0; n <= kt; n++) {
          ...
          // self-coinsistency:
          if (!atomic_limit) {
            DMFT_selfconsisteny(n, Gloc_up, Delta_up, J);
          }
    ...
    cout << "REALTIOME: nt= " << nt << " steps " << endl;
    for (tstp = kt + 1; tstp <= nt; tstp++) {
      ...
      for (iter = 1; iter <= iter_rtime; iter++) {
        ...
        if (!atomic_limit) {
          DMFT_selfconsisteny(tstp, Gloc_up, Delta_up, J);
        }
    ...
    vector<double> Ekin(nt + 2), current(nt + 2);
    get_Ekin(Ekin, current, Gloc_up, J, kt, beta, h);
    ...
    if (text_output) {
      ...
      ofile.precision(14);
      ofile << "# tstp <nup> <ndo> <ekin> <docc> <Eint> <current> <Q> A(t) "
               "E(t) intEj"
            << endl;
      double intEj = 0.0;
      double Et = 0;
      for (int tstp = -1; tstp <= nt; tstp++) {
        ...
        ofile << current[tstp + 1] << " ";
        ofile << imp.hamiltonian.Q_exp[tstp + 1] << " ";
        ofile << vector_potential[tstp + 1] << " ";
        if (tstp > 0) {
          // simple estimates for E=-dA/dt and
          // intj = int_0^t dt' j(t')*E(t')
          // assuming that E(0)=0
          double Et_new =
              -(vector_potential[tstp + 1] - vector_potential[tstp]) / h;
          intEj += 0.5 * h * (Et_new * current[tstp + 1] + Et * current[tstp]);
          Et = Et_new;
        } else {
          Et = 0;
          intEj = 0;
    ...
    // OBSERVABLES
    {
      ...
      store_real_data_to_hid(group_id, "current", current.data(),
                             current.size());
