...
#include "./ppsc/hamiltonians/single_band_hubbard_spin.hpp"
...
ppsc::pp_ints_type get_pp_ints(ppsc::gf_type & Delta,
  ...
  pp_ints.push_back(ppsc::pp_int_type(Delta,    0, 0, cuc, cua, fermion, fwd)); // spin up fwd; hybridization reads S_Delta = - (???) int_time cuc(t) Delta(t,t') cua(t')  dt dt'
  pp_ints.push_back(ppsc::pp_int_type(Delta_cc, 0, 0, cua, cuc, fermion, bwd)); // spin up bwd

  pp_ints.push_back(ppsc::pp_int_type(Delta,    0, 1, cuc, cda, fermion, fwd)); // spin up do fwd
  pp_ints.push_back(ppsc::pp_int_type(Delta_cc, 0, 1, cda, cuc, fermion, bwd)); // spin up do bwd

  pp_ints.push_back(ppsc::pp_int_type(Delta,    1, 0, cdc, cua, fermion, fwd)); // spin do up fwd
  pp_ints.push_back(ppsc::pp_int_type(Delta_cc, 1, 0, cua, cdc, fermion, bwd)); // spin do up bwd
  
  pp_ints.push_back(ppsc::pp_int_type(Delta,    1, 1, cdc, cda, fermion, fwd)); // spin do fwd 
  pp_ints.push_back(ppsc::pp_int_type(Delta_cc, 1, 1, cda, cdc, fermion, bwd)); // spin do bwd 
...
ppsc::gf_verts_type get_gf_verts(HILB & hil_) {
  ...
  gf_verts.push_back(ppsc::gf_vert_type(0, 1, cua, cdc)); // spin up do
  gf_verts.push_back(ppsc::gf_vert_type(1, 0, cda, cuc)); // spin do up

  gf_verts.push_back(ppsc::gf_vert_type(1, 1, cda, cdc)); // spin do
  return gf_verts;
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    typedef ppsc::hamiltonians::single_band_hubbard_spin<hilbert_space_type> hamiltonian_type;
    ...
    { // -- Setup local Hamiltonian
      ...
      find_param_tvector(argv[1], "__Bx=", imp.hamiltonian.Bx, nt);
      find_param_tvector(argv[1], "__By=", imp.hamiltonian.By, nt);
      find_param_tvector(argv[1], "__Bz=", imp.hamiltonian.Bz, nt);

      find_param(argv[1], "__Bx0=", imp.hamiltonian.Bx[0]);
      find_param(argv[1], "__By0=", imp.hamiltonian.By[0]);
	...
	// **** kann auch nur einmal am Anfang aufgerufen werden, da hier nur Delta, Delta_cc und hil_ als Referenzen/Pointer übergeben werden und keine Daten überschrieben werden!!!!
	ppsc::pp_ints_type pp_ints = get_pp_ints(Delta, Delta_cc, imp.hamiltonian.hil_);
	ppsc::gf_verts_type gf_verts = get_gf_verts(imp.hamiltonian.hil_);
	// ****
