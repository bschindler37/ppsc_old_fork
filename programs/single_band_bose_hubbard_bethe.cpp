...
#include "./ppsc/hamiltonians/single_band_bose_hubbard.hpp"
...
ppsc::pp_ints_type get_pp_ints(ppsc::gf_type & Delta,
  ...
  // no sign change on reversal for bosons
  int boson=+1, fermion=-1, fwd=+1, bwd=-1;

  int xi = Delta.sig();
  
  ppsc::pp_ints_type pp_ints;
  pp_ints.push_back(ppsc::pp_int_type(Delta,    cc, ca, xi, fwd));
  pp_ints.push_back(ppsc::pp_int_type(Delta_cc, ca, cc, xi, bwd));
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // READ GENERAL INPUT (NOT YET MICROSCOPIC PARAMETERS)
    {
      ...
      find_param(argv[1], "__Nmax=", Nmax);
    }
    // ---------------------------------------------------------------------
    // -- Setup pp calculator

    typedef ppsc::hilbert_spaces::single_band_bose_diag hilbert_space_type;
    typedef ppsc::hamiltonians::single_band_bose_hubbard<hilbert_space_type> hamiltonian_type;
    typedef ppsc::solver<hamiltonian_type> solver_type;
    
    hilbert_space_type hilbert_space;
    hilbert_space.init(Nmax);
    ...
    int dim=1;
    int boson=+1, fermion=-1;
    cntr::herm_matrix<double> Gloc(nt, ntau, dim, boson);    
    cntr::herm_matrix<double> Delta(nt, ntau, dim, boson);
    cntr::herm_matrix<double> Delta_cc(nt, ntau, dim, boson);
    ...
    {
      cntr::herm_matrix<double> gtmp(-1, ntau, 1, boson);
    ...
    if (nt > 0 && matsubara_converged == true) {
      ...
      cntr::herm_matrix<double> gtmp(kt, ntau, 1, boson);
      imp.init_real_time();
    ...
    std::vector<double> Ekin =
      ppsc::get_kinetic_energy(Gloc, Delta, beta, h, kt);
