...
public:
  ...
  void update(int tstp, single_particle_greensfunction_type & Gloc,
	      cntr::function<double> & tfunc_sin,
	      cntr::function<double> & tfunc_cos,
	      int af_bethe_flag=0) {
    ...
    */
    ...
    if(af_bethe_flag == 1) _component_update(tstp, this->o1up, this->o1up_cc, Gloc.o1do, tfunc_sin, tfunc_cos);
    else                   _component_update(tstp, this->o1up, this->o1up_cc, Gloc.o1up, tfunc_sin, tfunc_cos);
    ...
    */
    ...
    if(af_bethe_flag == 1) _component_update(tstp, this->o1do, this->o1do_cc, Gloc.o1up, tfunc_sin, tfunc_cos);
    else                   _component_update(tstp, this->o1do, this->o1do_cc, Gloc.o1do, tfunc_sin, tfunc_cos);
    ...
    */
    ...
    if(af_bethe_flag == 1) _component_update(tstp, this->o2up, this->o2up_cc, Gloc.o2do, tfunc_sin, tfunc_cos);
    else                   _component_update(tstp, this->o2up, this->o2up_cc, Gloc.o2up, tfunc_sin, tfunc_cos);
    ...
    */
    ...
    if(af_bethe_flag == 1) _component_update(tstp, this->o2do, this->o2do_cc, Gloc.o2up, tfunc_sin, tfunc_cos);
    else                   _component_update(tstp, this->o2do, this->o2do_cc, Gloc.o2do, tfunc_sin, tfunc_cos);
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    { // -- Setup local Hamiltonian
      ...
      find_param(argv[1], "__phi0=", phi_vec[0]);
    ...
    cntr::function<double> tfunc_sin(nt);
    cntr::function<double> tfunc_cos(nt);
    for(int tstp=-1; tstp <= nt; tstp++) {
      tfunc_sin[tstp] = t_vec[tstp+1] * std::sin(phi_vec[tstp+1]);
      tfunc_cos[tstp] = t_vec[tstp+1] * std::cos(phi_vec[tstp+1]);
    }
	...
	Delta.update(-1, Gloc, tfunc_sin, tfunc_cos, af_bethe_flag);
    ...
    if(!imp.has_symmetries()) {
      imp.symmetry_reduction(-1, sym_tol);
      std::string filename = "sym_eq.txt";
    ...
    if (nt > 0 && matsubara_converged == true) {
	  ...
	  Delta.update(n, Gloc, tfunc_sin, tfunc_cos, af_bethe_flag);
        }
	...
	Delta.update(tstp, Gloc, tfunc_sin, tfunc_cos, af_bethe_flag);
    ...
    if(matsubara_converged && nt > 0 && !imp.has_symmetries()) {
      imp.symmetry_reduction(kt, sym_tol);
      std::string filename = "sym_rt.txt";
      imp.write_symmetries(filename);
    }
    ...
    // OBSERVABLES
    {
      ...
      store_real_data_to_hid(group_id, "phi_vec", phi_vec.data(), phi_vec.size());
