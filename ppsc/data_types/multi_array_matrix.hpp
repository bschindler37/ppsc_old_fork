...
public:
  ...
  multi_array_matrix_base(
    array_shape_type array_shape, flavour_shape_type flavour_shape) :
    array_shape(array_shape), flavour_shape(flavour_shape),
    shape(concatenate_arrays(array_shape, flavour_shape)),
    total_size(product_reduction(shape)), total_flavour_size(product_reduction(flavour_shape)),
    view_(NULL, shape) {

    assert( MAT::RowsAtCompileTime == -1 || MAT::RowsAtCompileTime == flavour_shape[0] );
    assert( MAT::ColsAtCompileTime == -1 || MAT::ColsAtCompileTime == flavour_shape[1] );
  }
  ...
  matrix_view_type get_matrix_view(array_shape_type array_index) {
    shape_type index(concatenate_arrays(array_index, flavour_shape_type({0, 0})));
    return matrix_view_type(& this->view()(index), flavour_shape[0], flavour_shape[1]);
  }
  */
  ...
  matrix_view_type get_matrix_view(array_shape_type_template<T> array_index,
  ...
  matrix_view_type get_matrix_view(array_shape_type_template<T> array_index,
    ...
    return matrix_view_type(
      data() + (array_shape[1]*array_index[0] + array_index[1]) * total_flavour_size,
      flavour_shape[0], flavour_shape[1]);
  }
  ...
  this_type & operator*=(value_type scalar) {
    for(int i = 0; i < view_.num_elements(); i++) view_.data()[i] *= scalar;
    return *this;
  }
  ...
  bool is_zero(double tol=1e-12) {
    for(int i = 0; i < view_.num_elements(); i++)
      if( std::abs(view_.data()[i]) > tol ) return false;
    return true;
  }
  ...
  // -- templated compare
  template<class T>
  bool is_equal_to(T & other_in, double tol=1e-12) {
    this_type other(static_cast<this_type>(other_in));
    if(view_.num_elements() != other.view().num_elements()) return false;
    for(int i = 0; i < view_.num_elements(); i++)
      if( std::abs(view_.data()[i] - other.view().data()[i]) > tol ) return false;
    return true;    
  }

  template<class T>
  double abs_diff(T & other_in) {
    double diff = 0.0;
    this_type other(static_cast<this_type>(other_in));

    if(view_.num_elements() != other.view().num_elements()) return -1.0;
    if(view_.num_elements() == 0) return 0.0;
...
class multi_array_matrix :
    public multi_array_matrix_base<array_dimensions, MAT,
    multi_array_matrix<array_dimensions, MAT>  >
{
public:
  ...
  multi_array_matrix(multi_array_matrix<array_dimensions, OTHER_MAT> && other) :
    base_type(other.get_array_shape(), other.get_flavour_shape()), buffer(std::move(other.buffer))    
  {

    static_assert( !((MAT::RowsAtCompileTime == OTHER_MAT::RowsAtCompileTime) &&
		     (MAT::ColsAtCompileTime == OTHER_MAT::ColsAtCompileTime)),
		   "multi_array_matrix Move ctor from templated matrix used for the same type. Should Not Happen." );

    constexpr int dynamic = -1;
    
    if( (OTHER_MAT::RowsAtCompileTime == dynamic) && (MAT::RowsAtCompileTime != dynamic) )
      assert( MAT::RowsAtCompileTime == other.get_flavour_shape()[0] );
    
    if( (OTHER_MAT::ColsAtCompileTime == dynamic) && (MAT::ColsAtCompileTime != dynamic) )
      assert( MAT::ColsAtCompileTime == other.get_flavour_shape()[1] );
  }
    */
...
public:
  ...
  multi_array_matrix_ref(value_type * ptr,
			 array_shape_type array_shape,
			 flavour_shape_type flavour_shape) :
  ...
  template<class OTHER_MAT>
  multi_array_matrix_ref(const multi_array_matrix_ref<array_dimensions, OTHER_MAT> & other) :
    multi_array_matrix_ref(other.data(), other.get_array_shape(), other.get_flavour_shape())
  {

    static_assert( !( (static_cast<int>(MAT::RowsAtCompileTime) == static_cast<int>(OTHER_MAT::RowsAtCompileTime)) &&
		      (static_cast<int>(MAT::ColsAtCompileTime) == static_cast<int>(OTHER_MAT::ColsAtCompileTime)) ),
		   "multi_array_matrix_ref template copy ctor used for the same type. Should Not Happen." );

    constexpr int dynamic = -1;

    assert( ( (OTHER_MAT::RowsAtCompileTime == dynamic) && (MAT::RowsAtCompileTime != dynamic) ) ? // if
	    ( MAT::RowsAtCompileTime == other.get_flavour_shape()[0] ) // then check this
	    : true && "multi_array_matrix_ref: dynamic to static rows cast failed." );

    assert( ( (OTHER_MAT::ColsAtCompileTime == dynamic) && (MAT::ColsAtCompileTime != dynamic) ) ? // if
	    ( MAT::ColsAtCompileTime == other.get_flavour_shape()[1] ) // then check this
	    : true && "multi_array_matrix_ref: dynamic to static cols cast failed.");     
  }
...
public:
  ...
  template<class A = T>
  array_matrix_adaptor(value_type * ptr, size_type size, size_type rows, size_type cols,
		       typename std::enable_if<!A::has_data>::type * = 0) :
  ...
  */
  ...
  matrix_view_type operator() (size_type index) { return base_type::get_matrix_view({{index}}); }
...
// -- array_matrix
template<class M> using array_matrix =
  array_matrix_adaptor< multi_array_matrix<1, M> >;

// -- array_matrix_ref
template<class M> using array_matrix_ref =
  array_matrix_adaptor< multi_array_matrix_ref<1, M> >;

// -----------------------------------------------------------------------

template<class ARR>
array_matrix_ref<typename ARR::matrix_type>
get_view_from(size_type idx, ARR & array) {
...
template<class MAT>
array_matrix_ref<typename MAT::matrix_type>
get_column_vector(size_type idx, MAT & matrix) {

  assert( idx >= 0 );
  assert( idx < matrix.get_array_shape()[0] );

  static_assert( MAT::dimensions == 4, "get_column_vector called with non-matrix" );

  return array_matrix_ref<typename MAT::matrix_type>
    (& matrix[idx][0][0][0],
     matrix.get_array_shape()[1],
...
public:
  ...
  template<class A = T>
  matrix_matrix_adaptor(size_type size1, size_type size2, size_type rows, size_type cols,
		       typename std::enable_if<A::has_data>::type * = 0) :
...
// -- matrix_matrix
template<class M> using matrix_matrix =
  matrix_matrix_adaptor< multi_array_matrix<2, M> >;

// -- matrix_matrix_ref
template<class M> using matrix_matrix_ref =
  matrix_matrix_adaptor< multi_array_matrix_ref<2, M> >;
  
// -----------------------------------------------------------------------
// -- triangular types
  
template<class GETTER> class triangular_matrix_matrix_base :
    public GETTER {
...
public:
  ...
  template<class T> triangular_matrix_matrix_base(const T & other) : base_type(other) {}
  
  template<class A = base_type>
  triangular_matrix_matrix_base(size_type size, size_type rows, size_type cols,
		       typename std::enable_if<A::has_data>::type * = 0) :
    base_type({{size*(size+1)/2}}, {{rows, cols}})
  {}

  template<class A = base_type>
  triangular_matrix_matrix_base(value_type * ptr, size_type size, size_type rows, size_type cols,
		       typename std::enable_if<!A::has_data>::type * = 0) :
  ...
  template<class A = GETTER>
  matrix_type operator() (size_type index1, size_type index2,
...
template<class M, class IDX> class plain_getter :
    public array_matrix_ref<M>, IDX {
public:
...
template<class M, class IDX> class hermitian_getter :
    public array_matrix_ref<M>, IDX {
public:
  ...
  matrix_type operator() (size_type index1, size_type index2) const {
    if(idx_type::allowed_range(index1, index2)) {
      ...
      return -matrix_type(const_cast<hermitian_getter<M, IDX> *>(this)->
			  base_type::get_matrix_view({{index}})).adjoint();
...
using hermitian_upper_triangular_matrix_matrix_ref =
  triangular_matrix_matrix_base< hermitian_getter<M, upper_triangular_indexer> >;
...
using hermitian_lower_triangular_matrix_matrix_ref =
  triangular_matrix_matrix_base< hermitian_getter<M, lower_triangular_indexer> >;
...
public:
  ...
  vt_from_tv_matrix_matrix_adaptor(
    value_type * ptr, size_type nt, size_type ntau, size_type rows, size_type cols, int sig,
    typename std::enable_if<!A::has_data>::type * = 0) :
  ...
  matrix_type operator() (size_type tau, size_type t) {
    ...
    return -sig * matrix_type(base_type::get_matrix_view({{t, rev_tau}})).adjoint();
  }
...
public:
  ...
  vt_from_tv_matrix_matrix_ref(const vt_from_tv_matrix_matrix_ref<MOTHER> & other) :
...
public:
  ...
  gtr_from_ret_and_less_matrix_matrix_ref(
    value_type * ret_ptr, value_type * les_ptr,
    size_type nt, size_type rows, size_type cols) :
