...
#ifndef _PPSC_HAMILTONIANS_two_band_tJ_pos_HPP
#define _PPSC_HAMILTONIANS_two_band_tJ_pos_HPP

// -----------------------------------------------------------------------
//
// Two band tJ-like Hamiltonian builder
//
// Author: Denis Golez denis.golez@gmail.com (July 2018)
//
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
template<class HILB> class two_band_tJ_pos {

public:

  // ---------------------------------------------------------------------
  two_band_tJ_pos(int nt, HILB & hilbert_space) :
    nt(nt),
    ...
    n1u=hil_.n_op_[hil_.flavor(0, 0, 0)];
    n1d=hil_.n_op_[hil_.flavor(0, 1, 0)];

    n2u=hil_.n_op_[hil_.flavor(1, 0, 0)];
    n2d=hil_.n_op_[hil_.flavor(1, 1, 0)];

    n1 = n1u + n1d;
    n2 = n2u + n2d;

    n = n1 + n2;
    // std::cout << "N operator " << n << std::endl; 
    m = n1u - n1d + n2u - n2d; // magnetization
    // std::cout << "m operator " << m << std::endl;
    densdens_intraband = n1u * n1d + n2u * n2d;
    // std::cout << "Denden-intra operator " << densdens_intraband << std::endl;
    densdens_interband = n1 * n2;
    // std::cout << "Denden-inter operator " << densdens_interband << std::endl;
    densdens_interband_equalspin = n1u * n2u + n1d * n2d;
    // std::cout << "densdens_interband_equalspin " << densdens_interband_equalspin << std::endl;
  ...
  void get_hamiltonian(int tstp, operator_type & Htemp) {
    ...
    // std::cout << "half_filling_shift,mu, d, u, j = " << half_filling_shift << ", " << mu << ", " << d << ", " << u << ", " << j << std::endl;
...
#endif // _PPSC_HAMILTONIANS_two_band_tJ_pos_HPP
