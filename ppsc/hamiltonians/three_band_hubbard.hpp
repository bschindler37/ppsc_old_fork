...
#ifndef _PPSC_HAMILTONIANS_THREE_BAND_HUBBARD_HPP
#define _PPSC_HAMILTONIANS_THREE_BAND_HUBBARD_HPP
...
public:
  ...
  // ---------------------------------------------------------------------
  three_band_hubbard(int nt, HILB & hilbert_space) :
    nt(nt),
    mu(0.0), sosm_seed(0.0),
    // -- parameter vectors
    ...
    n1_exp(nt+2), n2_exp(nt+2), n3_exp(nt+2), m_exp(nt+2),
  ...
  void update_exp_vals(int tstp, operators_type & rho) {
    ...
    std::cout << "tstp, n, d1, d2, d3, m, Q = "
	      << tstp << ", "
	      << n1_exp[tstp + 1]+n2_exp[tstp + 1]+n3_exp[tstp + 1] << ", "
              << d1_exp[tstp + 1] << ", "
              << d2_exp[tstp + 1] << ", "
              << d3_exp[tstp + 1] << ", "
	      << m_exp[tstp + 1] << ", "
	      << Q_exp[tstp + 1] << ", n1, n2, n3, "
	      << n1_exp[tstp + 1] << ", "
	      << n2_exp[tstp + 1] << ", "
	      << n3_exp[tstp + 1] << std::endl;
  }
