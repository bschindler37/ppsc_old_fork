...
/*!
   ...
   have the same encoding in a document)
    \tparam Allocator   Allocator type for allocating memory of object, array
   and string.
...
template <typename CharType> struct GenericStringRef {
  ...
  /*! \param str constant string, lifetime assumed to be longer than the use of
     the string in e.g. a GenericValue
...
public:
  ...
  */
  ...
  explicit GenericValue(T b,
                        CEREAL_RAPIDJSON_ENABLEIF((internal::IsSame<bool, T>)))
  ...
  */
  ...
  /*! \param str Constant string reference to be assigned
     ...
     type assignment overload below.
  ...
  */
  ...
  /*! \pre IsObject() == true
     ...
     with \ref operator[](SizeType))
      \note In version 0.1x, if the member is not found, this function returns a
     null value. This makes issue 7. Since 0.2, if the name is not correct, it
     will assert. If user is unsure whether a member exists, user should use
  ...
  */
  ...
  /*!
     ...
     handle string with null character.
      \param name Member name to be searched.
      \pre IsObject() == true
      \return Whether a member with that name exists.
      \note It is better to use FindMember() directly if you need the obtain the
  ...
  */
  ...
  /*! \param name A string value as name of member.
     ...
     one as used before. Commonly use GenericDocument::GetAllocator().
  ...
  */
  ...
  /*! \param name A string value as name of member.
     ...
     one as used before. Commonly use GenericDocument::GetAllocator().
      ...
      \pre  IsObject()
      \note This overload is needed to avoid clashes with the generic primitive
     type AddMember(GenericValue&,T,Allocator&) overload below.
      \note Amortized Constant time complexity.
  */
  ...
  */
  ...
  /*! \param name A constant string reference as name of member.
     ...
     one as used before. Commonly use GenericDocument::GetAllocator().
      ...
      \pre  IsObject()
      \note This overload is needed to avoid clashes with the generic primitive
     type AddMember(StringRefType,T,Allocator&) overload below.
      \note Amortized Constant time complexity.
  */
  ...
  */
  ...
  /*! \param value        Constant string reference to be appended.
      \param allocator    Allocator for reallocating memory. It must be the same
     one used previously. Commonly use GenericDocument::GetAllocator().
      \pre IsArray() == true
      \return The value itself for fluent API.
      \note If the number of elements to be appended is known, calls Reserve()
     once first may be more efficient.
...
private:
  ...
  // for CEREAL_RAPIDJSON_48BITPOINTER_OPTIMIZATION=1 inline (for `UTF8`-encoded
  // strings).
  struct ShortString {
    enum {
...
/*!
   ...
   parsing.
    \warning Although GenericDocument inherits from GenericValue, the API does
   \b not provide any virtual functions, especially no virtual destructor.  To
   avoid memory leaks, do not \c delete a GenericDocument object via a pointer
   to a GenericValue.
*/
template <typename Encoding, typename Allocator = MemoryPoolAllocator<>,
